...
from gempython.tools.amc_user_fuctions_xhal import *
...
def getVFATObject(cardName,addrTable=None,debug=False):
    if addrTable is None:
        addrTable = "{0}/amc_address_table_top.xml".format(os.getenv("GEM_ADDRESS_TABLE_PATH"))

    vfat3board = VFAT3(cardName,addrTable)

    if debug:
        print("FW release major = {0}".format(vfat3board.readReg("GEM_AMC.GEM_SYSTEM.RELEASE.MAJOR"))

    return vfat3board

def configureVFAT3DacMonitorMulti(vfat3board, dacSelect, ohMask=0xFFF, amcboard=None):
    """
    ...
    vfat3board - instance of xhalpy.VFAT3
    ...
    if amcboard is None:
        amcboard = getAMCObject(cardName=vfat3board.getBoardName())

    ohVFATMaskArray = amcboard.getMultiLinkVFATMask(ohMask)
    vfat3board.confDacMonitorMulti(ohMask, ohVFATMaskArray, dacSelect)
    return
